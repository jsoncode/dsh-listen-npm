...
function openStore(raw: string): NpmStoreData {
  ...
  const watch: WatchEntry[] = Array.isArray(parsed.watch)
    ? (parsed.watch as Array<Record<string, unknown>>)
      .map((w) => ({
        name: String(w.name || ''),
  ...
  if (parsed.snapshots && typeof parsed.snapshots === 'object' && !Array.isArray(parsed.snapshots)) {
    for (const [key, list] of Object.entries(parsed.snapshots as Record<string, unknown>)) {
      ...
      if (items.length > 0) snapshots[key] = items.slice(0, SNAPSHOTS_LIMIT)
    }
  }
...
export async function loadStore(dir: string): Promise<NpmStoreData | null> {
  ...
  try {
    ...
    try { await rename(target, target + '.bak') } catch { /* 备份失败忽略 */ }
...
function doSave(dir: string, store: NpmStoreData): Promise<void> {
  return (async () => {
