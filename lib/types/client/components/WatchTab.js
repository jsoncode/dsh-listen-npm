...
import { fmtCompact, fmtInt, fmtRel } from "../format.js";
...
function MiniTrend({ daily }) {
    ...
    return (_jsx("div", { className: "dshn-trend", title: t('watchTrend'), "aria-label": t('watchTrend'), children: data.map((p, i) => (_jsx("span", { className: 'dshn-trend-bar' + (i === data.length - 1 ? ' dshn-trend-bar-last' : ''), style: { height: Math.max(10, Math.round((p.downloads / max) * 100)) + '%' }, title: `${p.day} · ${fmtInt(p.downloads)}` }, p.day))) }));
}
