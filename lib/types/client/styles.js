...
export const css = [
    ...
    // 弹框「监控」tab 里开关行紧跟监控工具栏，单独补一档下间距（设置分区页由
    // .dshn-settings 的 gap 负责，不受此规则影响）
    '.dshn-modal-body .dshn-pref:first-child{margin-bottom:12px}',
    // ── 弹框骨架（配色对齐 dsh-get-balance：蒙版 blur(12px) saturate(1.2)、
    //    弹框 78% bg-layer-1 玻璃、14px 圆角、同一投影）─────────────────
    ...
    // 版本变更徽章：宿主 state-warn-primary 琥珀底 + 深琥珀字（比白字对比度更高，
    // 与 dsh-jenkins 的构建中胶囊同款处理）
