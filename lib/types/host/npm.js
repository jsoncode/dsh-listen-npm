...
export function platformSlugOf(url) {
...
export function issuesUrlOfPlatform(host, slug) {
 ...
 */
export function deriveRepoAndIssues(repoRaw, bugsRaw) {
    let repository;
    ...
    const bugsCandidate = typeof bugsRaw === 'string'
        ? bugsRaw
        : bugsRaw && typeof bugsRaw === 'object' && typeof bugsRaw.url === 'string'
            ? String(bugsRaw.url)
            : undefined;
    let bugs;
...
async function npmGet(deps, url) {
    ...
    if (sub === undefined)
        throw new NpmError('subprocess-missing', 'subprocess service unavailable');
    let curlPath;
    try {
...
async function npmGetJson(deps, url, source) {
    ...
    if (res.status === 404)
        throw new NpmError('pkg-not-found', 'HTTP 404: ' + url, 404);
    if (res.status === 429)
        throw new NpmError('rate-limited', 'HTTP 429: rate limited', 429);
    if (res.status >= 400)
        throw new NpmError('registry-http', 'HTTP ' + res.status + ': ' + url, res.status);
    let parsed;
    try {
    ...
    if (source === 'downloads' && parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
        ...
        if (typeof err === 'string' && err.length > 0)
            throw new NpmError('pkg-not-found', err, 404);
    }
    return parsed;
}
 ...
 */
export async function fetchPackageInfo(deps, name) {
    const doc = await npmGetJson(deps, `${registryBase(deps)}/${registryEnc(name)}`, 'registry');
    if (!doc || typeof doc !== 'object')
        throw new NpmError('parse-failed', 'registry doc must be an object');
    if (typeof doc.name !== 'string' || doc.name.length === 0)
        throw new NpmError('pkg-not-found', 'package not found: ' + name, 404);
    ...
    const author = typeof authorRaw === 'string'
        ? authorRaw
        : authorRaw && typeof authorRaw === 'object' && typeof authorRaw.name === 'string'
            ? String(authorRaw.name)
            : undefined;
    ...
    const keywords = Array.isArray(doc.keywords)
        ? doc.keywords.filter((k) => typeof k === 'string').slice(0, 30)
        : undefined;
    const info = {
        name: doc.name,
        ...
        latestDetail: latestManifest === undefined
            ...
            : {
                ...
                npmUser: latestManifest._npmUser && typeof latestManifest._npmUser.name === 'string'
                    ? String(latestManifest._npmUser.name)
                    : undefined,
            },
...
export async function fetchDailyRange(deps, name, range) {
    ...
    for (const item of rawList) {
        ...
        if (typeof o.day === 'string')
            downloads.push({ day: o.day, downloads: typeof o.downloads === 'number' ? o.downloads : 0 });
    }
    return {
...
export async function fetchPoint(deps, name, period) {
 ...
 * 分隔逗号保持原样（服务端按裸逗号切分）。
 */
export async function fetchBulkPoints(deps, names, period) {
    if (names.length === 0)
        return {};
    if (names.some((n) => n.startsWith('@')))
        throw new NpmError('bulk-scoped', 'bulk downloads API does not support scoped packages');
    const joined = names.map((n) => encodeURIComponent(n)).join(',');
    const parsed = await npmGetJson(deps, `${downloadsBase(deps)}/point/${period}/${joined}`, 'downloads');
    const out = {};
...
export async function searchPackages(deps, text, size) {
    ...
    for (const obj of objects) {
        ...
        const links = (pkg.links && typeof pkg.links === 'object' ? pkg.links : {});
        out.push({
            name: typeof pkg.name === 'string' ? pkg.name : '',
            version: typeof pkg.version === 'string' ? pkg.version : undefined,
            description: typeof pkg.description === 'string' ? pkg.description : undefined,
            keywords: Array.isArray(pkg.keywords) ? pkg.keywords.filter((k) => typeof k === 'string').slice(0, 8) : undefined,
