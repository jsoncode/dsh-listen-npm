...
const css = [
	...
	".dshn-btn-primary{background:var(--dsw-alias-button-primary-fill,var(--dsw-alias-brand-primary,#1668e3));border-color:transparent;color:var(--dsw-alias-label-primary-foreground,#fff)}",
	".dshn-btn-primary:hover:not(:disabled){background:var(--dsw-alias-button-primary-hover,var(--dsw-alias-brand-primary,#1668e3))}",
	...
	".dshn-input,.dshn-select{width:100%;box-sizing:border-box;background:var(--dsw-alias-bg-base,#fff);color:var(--dsw-alias-label-primary,#222);border:1px solid var(--dsw-alias-border-l2,#ccc);border-radius:8px;padding:8px 12px;font-size:13px;font-family:inherit;transition:border-color .15s,box-shadow .15s}",
	...
	".dshn-settings{display:flex;flex-direction:column;gap:12px}",
	".dshn-pref{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 12px;border:1px solid var(--dsw-alias-border-l2,#ddd);border-radius:10px;background:color-mix(in srgb,var(--dsw-alias-bg-base,#fff) 60%,transparent)}",
	".dshn-pref-text{min-width:0;display:flex;flex-direction:column;gap:2px}",
	".dshn-pref-label{font-size:13px;font-weight:600;color:var(--dsw-alias-label-primary,#222)}",
	".dshn-pref-desc{font-size:12px;color:var(--dsw-alias-label-secondary,#888)}",
	".dshn-switch{position:relative;display:inline-block;width:40px;height:22px;padding:0;border:none;border-radius:999px;background:color-mix(in srgb,var(--dsw-alias-label-primary,#222) 42%,transparent);cursor:pointer;flex:none;transition:background-color .2s}",
	".dshn-switch:hover{background:color-mix(in srgb,var(--dsw-alias-label-primary,#222) 55%,transparent)}",
	".dshn-switch:focus-visible{outline:none;box-shadow:0 0 0 3px color-mix(in srgb,var(--dsw-alias-brand-primary,#1668e3) 18%,transparent)}",
	".dshn-switch-on{background:#16a34a}",
	".dshn-switch-on:hover{background:#117f39}",
	".dshn-switch-knob{position:absolute;top:2px;left:2px;width:18px;height:18px;border-radius:50%;background:#fff;box-shadow:inset 0 0 0 1px rgba(0,0,0,.06),0 1px 4px rgba(0,0,0,.35);transition:left .2s cubic-bezier(.25,.8,.35,1)}",
	".dshn-switch-on .dshn-switch-knob{left:20px}",
	".dshn-pref-open{display:flex;gap:8px}",
	".dshn-modal-body .dshn-pref:first-child{margin-bottom:12px}",
	".dshn-backdrop{position:fixed;inset:0;z-index:1000;background:rgba(0,0,0,.32);display:flex;align-items:center;justify-content:center;-webkit-backdrop-filter:blur(12px) saturate(1.2);backdrop-filter:blur(12px) saturate(1.2)}",
	".dshn-modal{background:color-mix(in srgb,var(--dsw-alias-bg-layer-1,#fff) 78%,transparent);color:var(--dsw-alias-label-primary,#222);border:1px solid var(--dsw-alias-border-l2,#ddd);border-radius:14px;box-shadow:0 16px 48px rgba(0,0,0,.28);width:min(980px,94vw);height:min(760px,88vh);display:flex;flex-direction:column;overflow:hidden;-webkit-backdrop-filter:blur(24px) saturate(1.5);backdrop-filter:blur(24px) saturate(1.5)}",
	...
	".dshn-search-pop{position:absolute;top:calc(100% + 4px);left:0;right:0;z-index:30;background:var(--dsw-alias-bg-layer-1,#fff);border:1px solid var(--dsw-alias-border-l2,#ddd);border-radius:10px;box-shadow:0 10px 30px rgba(0,0,0,.18);max-height:300px;overflow-y:auto}",
	...
	".dshn-watch-item{display:flex;align-items:center;gap:12px;padding:10px 12px;border:1px solid var(--dsw-alias-border-l1,#eee);border-radius:10px;margin-bottom:8px;background:var(--dsw-alias-bg-layer-2,#fafafa)}",
	...
	".dshn-note-version{color:#4a3500;background:var(--dsw-alias-state-warn-primary,#f59e0b);border-color:transparent}",
	...
	"@supports not ((-webkit-backdrop-filter: blur(1px)) or (backdrop-filter: blur(1px))){.dshn-modal{background:var(--dsw-alias-bg-layer-1,#fff)}}"
];
/**
* 幂等注入样式。
*
...
function createPoller(run, summary) {
	...
	return {
		...
		markSeen() {
			if (!cachedWatch.some((w) => w.hasNewVersion === true)) return;
			applyWatch(cachedWatch.map((w) => w.hasNewVersion === true ? {
				...w,
				hasNewVersion: false
			} : w));
			run("", { op: "watchSeen" }).catch(() => {});
		},
		invalidate() {
			dirty = true;
			loadWatch();
...
const NPM_LOGO = "data:image/svg+xml," + encodeURIComponent("<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"64\" height=\"64\" viewBox=\"0 0 64 64\"><rect width=\"64\" height=\"64\" rx=\"14\" fill=\"#CB3837\"/><text x=\"32\" y=\"41.5\" font-family=\"Arial, Helvetica, sans-serif\" font-size=\"23\" font-weight=\"700\" fill=\"#ffffff\" text-anchor=\"middle\" letter-spacing=\"0.5\">npm</text></svg>");
/**
...
function FooterButton({ onOpen, reportSession, wide = false, useSessions, poller }) {
	const visible = (0, react.useSyncExternalStore)(showInMenuStore.subscribe, showInMenuStore.getSnapshot, showInMenuStore.getSnapshot);
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		}), showWatch ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
...
//#endregion
//#region src/client/components/ShowInMenuToggle.tsx
/**
...
function ShowInMenuToggle() {
	const on = (0, react.useSyncExternalStore)(showInMenuStore.subscribe, showInMenuStore.getSnapshot, showInMenuStore.getSnapshot);
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		className: "dshn-pref",
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			className: "dshn-pref-text",
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dshn-pref-label",
				children: t("showInMenu")
			}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dshn-pref-desc",
				children: t("showInMenuDesc")
			})]
			...
			onClick: () => {
				showInMenuStore.set(!on);
			},
			children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { className: "dshn-switch-knob" })
		})]
	});
}
//#endregion
//#region src/client/components/PluginSettingsPage.tsx
/**
...
* 弹框，避免两层遮罩叠在一起）。
*/
function PluginSettingsPage({ onOpen, close }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		className: "dshn-settings",
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ShowInMenuToggle, {}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
			className: "dshn-pref-open",
...
function latestExpectedDay(now = /* @__PURE__ */ new Date()) {
...
*                  dataEnd 或非法时不做尾部补齐。
*/
function buildDailySeries(raw, targetEnd) {
	...
	const lagDays = isValidDay(targetEnd) && targetEnd > dataEnd ? Math.min(daysBetween(dataEnd, targetEnd), 14) : 0;
	...
	for (let day = dataStart; day <= last; day = shiftDay(day, 1)) {
		...
		if (value !== void 0) daily.push({
			day,
			downloads: value
		});
		else if (day > dataEnd) daily.push({
			day,
			downloads: 0,
			pending: true
		});
...
const seriesOf = (daily) => {
	const padded = buildDailySeries((Array.isArray(daily) ? daily : []).filter((p) => p.pending !== true), latestExpectedDay());
...
function DownloadChart({ data }) {
	...
	if (series.all.length === 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("svg", {
		...
		children: [
			...
			series.lagDays > 0 && realLastIdx >= 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("line", {
				className: "dshn-chart-split",
				x1: splitX,
				y1: PAD_T,
				x2: splitX,
				y2: 216
			...
			all.map((p, i) => {
				...
				const h = pending ? 2 : Math.max(1, 216 - yOf(p.downloads));
				const cls = "dshn-chart-bar" + (pending ? " dshn-chart-bar-pending" : "") + (!pending && i === realLastIdx ? " dshn-chart-bar-last" : "") + (!pending && i === realPeakIdx ? " dshn-chart-bar-peak" : "");
				const tip = pending ? `${p.day} · ${t("noDataYet")}` : `${p.day} · ${fmtInt(p.downloads)}${i === realPeakIdx ? " · " + t("peakDay") : ""}`;
			...
			realPeakIdx >= 0 && realPeakIdx !== realLastIdx ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("text", {
				className: "dshn-chart-text",
				x: xOf(realPeakIdx) + barW / 2,
				y: yOf(all[realPeakIdx].downloads) - 5,
				textAnchor: "middle",
				children: fmtCompact(all[realPeakIdx].downloads)
			}) : null,
			xTicks.map((i) => {
				const day = all[i].day;
			...
			children: [
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									...
									children: dataEnd || p?.day?.end || ""
								})
							]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "dshn-stat",
							title: p?.week ? `${p.week.start} ~ ${p.week.end}` : void 0,
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: "dshn-stat-label",
									children: t("dlWeek")
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "dshn-stat",
							title: p?.month ? `${p.month.start} ~ ${p.month.end}` : void 0,
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: "dshn-stat-label",
									children: t("dlMonth")
				...
				lagDays > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: "dshn-note",
					children: t("lagNote", {
						end: dataEnd || "—",
						n: lagDays,
						from: series.pendingFrom || "",
						to: series.pendingTo || ""
					})
				}) : null,
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [t("dailyChartTitle"), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							className: "dshn-hint",
							children: [series.all.length > 0 ? `${series.all[0].day} ~ ${series.all[series.all.length - 1].day}` : "", lagDays > 0 ? ` · ${t("chartPending")}` : ""]
						})]
...
function MiniTrend({ daily }) {
	...
	const title = series.lagDays > 0 ? t("watchTrendLag", {
		end: lastRealDay || "—",
		n: series.lagDays
	}) : t("watchTrend");
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		...
		title,
		"aria-label": title,
		children: data.map((p) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
			className: "dshn-trend-bar" + (p.pending === true ? " dshn-trend-bar-pending" : "") + (p.pending !== true && p.day === lastRealDay ? " dshn-trend-bar-last" : ""),
...
function WatchTab({ run, poller, onOpenDetail, refreshMinutes }) {
	...
	const dlText = (w) => {
		...
		return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
			...
			lagged ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: "dshn-hint",
				children: t("lagShort", { n: series.lagDays })
			}) : null
		] });
	};
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [
		/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ShowInMenuToggle, {}),
		/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
	...
	return {
		...
		apply(ctx) {
			...
			slots.inject("settings.section", () => slots.register({
				...
				order: 43,
				label: () => t("configBtn")
			}, (props) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PluginSettingsPage, {
				onOpen: openModal,
				close: typeof props.close === "function" ? props.close : () => {}
			})));
