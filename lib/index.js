...
function openStore(raw) {
	...
	const watch = Array.isArray(parsed.watch) ? parsed.watch.map((w) => ({
		...
		daily: Array.isArray(w.daily) ? w.daily.map((p) => ({
			...
			...p.pending === true ? { pending: true } : {}
		})).filter((p) => p.day.length > 0).slice(-21) : void 0,
...
function entryDataFromRange(range, targetEnd = latestExpectedDay()) {
	...
	return {
		day: real.length > 0 ? real[real.length - 1].downloads : 0,
		week: sumDownloads(real.slice(-7)),
		daily: series.daily
	};
}
...
async function opInfo(deps, rawPkg) {
	...
	const points = dataEnd.length > 0 ? {
		day: {
			downloads: real[real.length - 1].downloads,
			start: dataEnd,
			end: dataEnd
		},
		week: {
...
function apply(ctx, config = {}) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args) {
			...
			if (lagDays > 0) lines.push(`统计延迟: npm 尚未公布最后 ${lagDays} 天的下载量（数据截至 ${dataEnd}，日期区间已按 0 补齐以保持连续，不计入上方统计）`);
			if (daily.length > 0) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args) {
			...
			const total = daily.filter((p) => p.pending !== true).reduce((acc, p) => acc + p.downloads, 0);
			const dataEnd = typeof r.dataEnd === "string" ? r.dataEnd : "";
			const lagDays = typeof r.lagDays === "number" ? r.lagDays : 0;
