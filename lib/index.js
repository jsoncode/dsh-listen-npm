...
async function fetchPackageInfo(deps, name) {
	...
	if (repoUrl) repository = repoUrl.replace(/^git\+/, "").replace(/\.git$/, "").replace(/^git:\/\//, "https://").replace(/^ssh:\/\/git@/, "https://");
	const bugsRaw = doc.bugs;
	const bugs = typeof bugsRaw === "string" ? bugsRaw : bugsRaw && typeof bugsRaw === "object" && typeof bugsRaw.url === "string" ? String(bugsRaw.url) : void 0;
	...
	return { info: {
		...
		latestDetail: latestManifest === void 0 ? void 0 : {
			...
			tarball: typeof latestDist.tarball === "string" ? latestDist.tarball : void 0,
...
* 分隔逗号保持原样（服务端按裸逗号切分）。
*/
async function fetchBulkPoints(deps, names, period) {
	...
	const out = {};
	if (parsed && typeof parsed === "object") for (const [key, value] of Object.entries(parsed)) {
...
async function opWatchRefresh(deps, rawPkg) {
	...
	const pointFor = async (name, period) => {
		try {
			return await fetchPoint(d, name, period);
		} catch {
			return;
		}
	};
	const [tagResults, bulkDay, bulkWeek, ...scopedPoints] = await Promise.all([
		...
		fetchBulkPoints(d, plainNames, "last-day").catch(() => ({})),
		fetchBulkPoints(d, plainNames, "last-week").catch(() => ({})),
		...scopedNames.flatMap((n) => [pointFor(n, "last-day").then((p) => ({
			name: n,
			period: "last-day",
			point: p
		})), pointFor(n, "last-week").then((p) => ({
			name: n,
			period: "last-week",
			point: p
		}))])
	]);
	const scopedPointOf = (name, period) => scopedPoints.find((s) => s.name === name && s.period === period)?.point;
	for (const w of targets) {
		...
		const day = dayHit !== void 0 ? dayHit.downloads : w.lastDay ?? 0;
		const week = weekHit !== void 0 ? weekHit.downloads : w.lastWeek ?? 0;
